...
async function init() {
  ...
  map.on("style.load", async () => {
    ...
    map.addSource('pressure-source', {
      type: 'raster',
      tiles: ['/tilesets/pressure_msl/{z}/{x}/{y}.png'],
      tileSize: 512,
      minzoom: 0,
      maxzoom: 4,
    });
...
const description = `
<p>
  Visualizing global presure at mean sea level at 0.25° from Meteo France's ARPEGE forecast model with Basemapkit's Spectre style.
</p>
...
export default {
  name: "Global pressure MSL",
  description,
  shortDescription: "Visualizing global presure at mean sea level with Basemapkit's Spectre style.",
  projectId: "pressure-msl-colorized",
  linkText: "Explore",
